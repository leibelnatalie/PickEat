...
public class SignupActivity extends AppCompatActivity implements GoogleApiClient.OnConnectionFailedListener, View.OnClickListener {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    buildGoogleApiClient(buildGoogleSignIn());

    mAuth = FirebaseAuth.getInstance();

    mAuthListener = getAuth();
  }

  private FirebaseAuth.AuthStateListener getAuth() {
  ...
  private void initViews() {
    findViewById(R.id.signup_google_btn).setOnClickListener(this);
  }
  ...
  public void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    if (requestCode == RC_SIGN_IN) {
      ...
      if (result.isSuccess()) {
        ...
        Toast.makeText(SignupActivity.this, R.string.google_signin_failed, Toast.LENGTH_SHORT).show();
      }
    }
  }
  ...
  private GoogleSignInOptions buildGoogleSignIn() {
    // Configure Google Sign In
    return new GoogleSignInOptions.Builder(GoogleSignInOptions.DEFAULT_SIGN_IN)
        .requestIdToken(Constants.FIREBASE_ID_TOKEN)
        .requestEmail()
        .build();
  }
  ...
  private void saveUser(FirebaseUser user) {
    SharedPref.saveRegisteredUser(user);
  }
